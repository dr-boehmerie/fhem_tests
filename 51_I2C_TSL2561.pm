...
=head1 SYNOPSIS
	...
	$Id: 51_I2C_TSL2561.pm 6072 2014-06-05 18:39:15Z kaihs $
...
use constant {
	...
	# Clipping thresholds
	TSL2561_CLIPPING_13MS     =>4900,    # Max value at Ti 13ms = 5047,
	TSL2561_CLIPPING_101MS    =>37000,   # Max value at Ti 101ms = 37177,
	TSL2561_CLIPPING_402MS    =>65000,   # Max value at Ti 402ms = 65535,
...
sub I2C_TSL2561_Initialize($) {
	...
	$hash->{AttrList} = 'do_not_notify:0,1 showtime:0,1 ' .
	                    ...
	                    'gain:1,16 integrationTime:13,101,402 autoGain:0,1 autoIntegration:0,1 ' . $readingFnAttributes;
	$hash->{integrationTime} = TSL2561_INTEGRATIONTIME_13MS;
	$hash->{gain} = TSL2561_GAIN_1X;
	$hash->{autoGain} = 0;
	$hash->{autoIntegration} = 0;
}
...
sub I2C_TSL2561_Define($$) {
	...
	if ($i2cModulesLoaded) {
		if (-r $dev && -w $dev) {
			$hash->{devI2C} = HiPi::Device::I2C->new( 
				devicename	=> $dev,
				address		=> hex($address),
				busmode		=> 'i2c',
			);
			...
			my $package = '';
			$hash->{package} = $sensorId >> 4;
			if ($hash->{package} == TSL2561_PACKAGE_CS) {
				$package = 'CS';
			} else {
				$package = 'T/FN/CL';
			}
...
sub I2C_TSL2561_Attr (@) {
	...
	if ($attr eq 'poll_interval') {
		...
		$hash->{autoGain} = $autoGain;

	} elsif ($attr eq 'autoIntegration') {
		my $autoIntegration = (defined($val) && looks_like_number($val) && $val > 0) ? $val : 0;
		
		$hash->{autoIntegration} = $autoIntegration;
	}	
...
sub I2C_TSL2561_ReadWord($$) {
	...
	try {
		my @values = $hash->{devI2C}->bus_read($register, 2);
...
sub I2C_TSL2561_ReadByte($$) {
	...
	try {
		...
		my @bytes = $hash->{devI2C}->bus_read($register, 1);
		$retVal = $bytes[0];
...
sub I2C_TSL2561_Enable($) {
	...
	try {
		$hash->{devI2C}->bus_write( TSL2561_COMMAND_BIT | TSL2561_REGISTER_CONTROL, TSL2561_CONTROL_POWERON );
...
sub I2C_TSL2561_Disable($) {
	...
	try {
	        $hash->{devI2C}->bus_write( (TSL2561_COMMAND_BIT | TSL2561_REGISTER_CONTROL, TSL2561_CONTROL_POWEROFF) );
	} catch Error with {
...
sub I2C_TSL2561_GetData($) {
	...
	if ($hash->{integrationTime} == TSL2561_INTEGRATIONTIME_13MS) {
	    usleep(20000); # 15ms
	} elsif ($hash->{integrationTime} == TSL2561_INTEGRATIONTIME_101MS) {
	    usleep(150000); # 102ms
	} else {
	    usleep(500000); # 403ms
	}
...
sub I2C_TSL2561_SetIntegrationTime($$) {
	...
	#try {
		...
		Log3 $name, 5, "I2C_TSL2561_SetIntegrationTime: time is " . $time ;
                Log3 $name, 5, "I2C_TSL2561_SetIntegrationTime: gain is " . $hash->{gain};
	        $hash->{devI2C}->bus_write( (TSL2561_COMMAND_BIT | TSL2561_REGISTER_TIMING, $time | $hash->{gain}) );
	#} catch Error with {
	#	Log3 $name, 1, 'ERROR: I2C_TSL2561_SetIntegrationTime: i2c-bus_write failure';
	#}
        # Update value placeholders 
        $hash->{integrationTime} = $time;
...
sub I2C_TSL2561_SetGain($$) {
	...
	#try {
		...
		Log3 $name, 5, 'I2C_TSL2561_SetGain: time is ' . $hash->{integrationTime};
		$timing = $gain | $hash->{integrationTime};
		Log3 $name, 5, ' I2C_TSL2561_SetGain: timing is ' . sprintf("%x", $timing);
		# Update the timing register 
		my $ret = $hash->{devI2C}->bus_write( (TSL2561_COMMAND_BIT | TSL2561_REGISTER_TIMING, $timing) );
...
sub I2C_TSL2561_GetLuminosity($) {
	...
	if (!$hash->{autoGain} && !$hash->{autoIntegration}) {
	...
	my $hi = 0;
	my $lo = 0;
	my $it = $hash->{integrationTime};

	if ($hash->{autoGain}) {
		while (!$valid) {
			$it = $hash->{integrationTime};
			
			# Get the hi/low threshold for the current integration time
			if ($it == TSL2561_INTEGRATIONTIME_13MS) {			# 0
				$hi = TSL2561_AGC_THI_13MS;
				$lo = TSL2561_AGC_TLO_13MS;
			} elsif ($it == TSL2561_INTEGRATIONTIME_101MS) {	# 1
			...
			if (!$agcCheck) {
				if (($hash->{broadband} < $lo) && ($hash->{gain} == TSL2561_GAIN_1X)) {			# 0
					# debug
		            readingsSingleUpdate($hash, 'state', 'AutoGain16x', 1);
				...
				} elsif (($hash->{broadband} > $hi) && ($hash->{gain} == TSL2561_GAIN_16X)) {	# 16
					# debug
					readingsSingleUpdate($hash, 'state', 'AutoGain1x', 1);
	...
	if ($hash->{autoIntegration}) {
 		$valid = 0;
 		$agcCheck = 0;
		
		while (!$valid) {
			# Run a check if we haven't already done so ... 
			if (!$agcCheck) {
				$it = $hash->{integrationTime};
				...
				if ($it == TSL2561_INTEGRATIONTIME_13MS) {			# 0
					$lo = 8;
					$hi = TSL2561_CLIPPING_13MS;
				} elsif ($it == TSL2561_INTEGRATIONTIME_101MS) {	# 1
					$lo = 8;
					$hi = TSL2561_CLIPPING_101MS;
				} else {											# 2
					$lo = 8;
					$hi = TSL2561_CLIPPING_402MS;
				}
				
				# Return 0 lux if the sensor is saturated 
				if (($hash->{broadband} > $hi) || ($hash->{ir} > $hi)) {
					# saturated
					if ($it == TSL2561_INTEGRATIONTIME_13MS) {
						# cannot decrease time
					} elsif ($it == TSL2561_INTEGRATIONTIME_101MS) {
						# debug
					#	readingsSingleUpdate($hash, 'state', 'AutoIntegration13ms', 1);
						# decrease to 13ms
					#	I2C_TSL2561_SetIntegrationTime($hash, TSL2561_INTEGRATIONTIME_13MS);
						# Drop the previous conversion results 
					#	I2C_TSL2561_GetData($hash);
						...
						# debug
						readingsSingleUpdate($hash, 'state', 'AutoIntegration101ms', 1);
						# decrease to 101ms
						I2C_TSL2561_SetIntegrationTime($hash, TSL2561_INTEGRATIONTIME_101MS);
						# Drop the previous conversion results 
						I2C_TSL2561_GetData($hash);
					}
					# Set a flag to indicate we've adjusted the integration time 
					$agcCheck = 1;
				} elsif (($hash->{broadband} < $lo) || ($hash->{ir} < $lo)) {
					# too low
					if ($it == TSL2561_INTEGRATIONTIME_13MS) {
						# debug
						readingsSingleUpdate($hash, 'state', 'AutoIntegration101ms', 1);
						...
						# debug
						readingsSingleUpdate($hash, 'state', 'AutoIntegration402ms', 1);
						# increase to 402ms
						I2C_TSL2561_SetIntegrationTime($hash, TSL2561_INTEGRATIONTIME_402MS);
						# Drop the previous conversion results 
						I2C_TSL2561_GetData($hash);
				...
				# integration time already adjusted, return the result
				$valid = 1;
			}
		}
	}
...
sub I2C_TSL2561_CalculateLux($) {
	my ($hash) = @_;
        ...
        if ($hash->{integrationTime} == TSL2561_INTEGRATIONTIME_13MS) {			# 0
            $clipThreshold = TSL2561_CLIPPING_13MS;
        } elsif ($hash->{integrationTime} == TSL2561_INTEGRATIONTIME_101MS) {	# 1
        ...
        } elsif ($hash->{saturated}) {
			# Reset "saturated"
			$hash->{saturated} = 0;
            readingsSingleUpdate($hash, 'state', 'Ok', 1);
		}

		# normal calculation using floating point
		my $ch0 = $hash->{broadband};
		my $ch1 = $hash->{ir};
		if ($ch0 > 0) {
			my $ratio = $ch1 / $ch0;
			my $lux = 0;
			...
			# scale according to integration time
			if ($hash->{integrationTime} == TSL2561_INTEGRATIONTIME_13MS) {			# 0
				$ch0 *= 322/11;
				$ch1 *= 322/11;
			} elsif ($hash->{integrationTime} == TSL2561_INTEGRATIONTIME_101MS) {	# 1
        ...
        if ($hash->{gain}==TSL2561_GAIN_1X) {	# 0
            $chScale = $chScale << 4;
        }
        ...
        } elsif ($hash->{package} == TSL2561_PACKAGE_T_FN_CL) {
            #  T, FN and CL package
