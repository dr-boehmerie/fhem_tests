...
#	$hash->{Timeout} = 200000;
	...
	if ($spiModulesLoaded) {
		if (-r $dev && -w $dev) {
			...
			#	$hash->{devBCM} = HiPi::BCM2835->new();
			#	$hash->{pinRST} = $hash->{devBCM}->get_pin( ST_RST );
			#	$hash->{pinRS}  = $hash->{devBCM}->get_pin( ST_RS );
			#	$hash->{pinPWM} = $hash->{devBCM}->get_pin( BL_PWM );
			...
			#	$hash->{pinPWM}->mode( RPI_PINMODE_OUTP );	# TODO: AltFunc!
				...
				Log3 $name, 3, "SPI_CBerry28_Define GPIO device created";
				...
				Log3 $name, 3, "SPI_CBerry28_Define pins exported";
				
			#	$hash->{pinRST} = $hash->{devGPIO}->get_pin( PIN_ST_RST );
			#	$hash->{pinRS}  = $hash->{devGPIO}->get_pin( PIN_ST_RS );
			#	$hash->{pinPWM} = $hash->{devGPIO}->get_pin( PIN_BL_PWM );
			#	Log3 $name, 3, "SPI_CBerry28_Define pins acquired";
				...
				# Control Pins using WiringPi
			#	HiPi::Wiring::wiringPiSetup();
			#	HiPi::Wiring::wiringPiSetupGpio();
			#	HiPi::Wiring::pinMode(ST_RST, WPI_OUTPUT);
			#	HiPi::Wiring::pinMode(ST_RS, WPI_OUTPUT);
			#	HiPi::Wiring::pinMode(BL_PWM, WPI_PWM_OUTPUT);
...
#			SPI_CBerry28_WriteByte($hash, 0, 0x80 + $hash->{pos});
		...
		Log3 $name, 1, 'ERROR: SPI_CBerry28_SetPos: spi-transfer failure';
	};
...
#			usleep(10);
		...
		Log3 $name, 1, 'ERROR: SPI_CBerry28_Output: spi-transfer failure';
	};
